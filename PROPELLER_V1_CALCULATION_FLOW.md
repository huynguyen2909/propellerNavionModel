...
```mermaid
flowchart TD
    A["RuntimeInput hiện tại"]
    B["lastLambdaInduced từ bước trước"]
    C["PropellerModel.evaluate"]
    D["Kiểm tra input và tính hub kinematics"]
    E{"enabled?"}
    F["Trả tải zero và trạng thái converged"]
    G["Đánh giá endpoints và quét miền để bracket nghiệm lambda_i"]
    G2{"Tìm được bracket?"}
    H["Giá trị thử lambda_i"]
    I["Full BEMT disk evaluation; nhận CT_r BE và disk loads"]
    J["Momentum theory; nhận CT_r MT"]
    K["R_lambda = CT_r BE - CT_r MT"]
    L{"abs R_lambda đạt tolerance?"}
    M["Newton step; nếu không an toàn thì bisection"]
    N{"Còn iteration?"}
    O["InflowStatus = RootNotBracketed"]
    O2["InflowStatus = MaximumIterations"]
    P["Dùng disk evaluation tại lambda_i cuối; gắn đầy đủ diagnostics"]
    Q["Đổi tải sang body; cộng arm và gyro moment"]
    R["PropellerOutput"]
    S{"output.inflow.converged?"}
    T["Cập nhật lastLambdaInduced bằng nghiệm mới"]
    U["Không ghi đè; giữ warm start trước đó"]
    V{"Chính sách của caller/application"}
    W["Takeoff demo: ném lỗi và dừng; không có step sau"]
    X["Caller khác: phải retry có thay đổi hoặc xử lý failure rõ ràng"]
    ...
    E -- "Có" --> G --> G2
    G2 -- "Không" --> O --> P
    G2 -- "Có; chọn warm start trong bracket" --> H
    H --> I --> K
    H --> J --> K
    K --> L
    L -- "Có" --> P
...
Hộp `Full BEMT disk evaluation` được chạy lại cho **mỗi** giá trị thử
