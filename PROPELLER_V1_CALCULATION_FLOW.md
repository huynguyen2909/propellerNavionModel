...
flowchart TD
    ...
    D["Kiểm tra runtime input"]
    E["Tính hub kinematics trong hệ propeller"]
    F{"enabled?"}
    G["Trả tải zero và trạng thái converged"]
    H["Giải uniform inflow lambda_i"]
    I["Disk loads tại lambda_i cuối"]
    J["Đổi tải sang body; cộng arm và gyro moment"]
    K["PropellerOutput"]
    L{"Inflow converged?"}
    M["Lưu lambda_i làm warm start"]
    N["Giữ warm start cũ"]
...
Trong code hiện tại, disk evaluation tạo ra lần đánh giá residual cuối được lưu
trong `ResidualEvaluation::disk`; vì vậy tải cuối được tái sử dụng, không chạy
BEMT thừa thêm một lần sau hội tụ.
